...
(function() {
    ...
    btn.onclick = function() {
        ...
        effectNodes.forEach(function(node) {
            ...
            var m = txt.match(/^(?:([a-zA-Záéíóúãõç\s]+):\s*)?\+(\d+)%\s+(.+)$/i);
            if (m) {
                var magnitude = parseInt(m[2]);
                var effectDesc = m[3].trim();
                ...
                if (m[1]) {
                    ...
                    // Efeitos globais (sem unidade específica, ex: +50% na força de defesa vs cerco)
                    params.push('bonus=' + encodeURIComponent('global|' + magnitude + '|' + effectDesc));
                }
            }
        });
